...
function AnimatedBars() {
	...
	return (
		<div className="flex w-auto items-end overflow-hidden">
			<span
				id="bar1"
				className="mr-[3px] h-2 w-1 bg-green-300 opacity-75 dark:bg-green-500"
			/>
			<span id="bar2" className="mr-[3px] h-1 w-1 bg-green-300 dark:bg-green-500" />
			<span id="bar3" className="h-3 w-1 bg-green-300 opacity-80 dark:bg-green-500" />
		</div>
	)
}
		...
		<>
			<div className="mb-2 flex items-center gap-2">
				<Image
					...
					priority
				/>
				<span className="w-full text-lg font-bold text-gray-800 dark:text-gray-100">
					{t('spotify.listening', { ns: 'common' })}{' '}
					{t('spotify.title', { ns: 'common' })}
				</span>
			</div>
				...
				<div className="inline-flex w-full max-w-full flex-col truncate sm:flex-row">
					{data?.songUrl ? (
						...
						<p className="capsize font-medium text-gray-800 dark:text-gray-200">
							{t('spotify.notPlaying', { ns: 'common' })}
						</p>
					)}
