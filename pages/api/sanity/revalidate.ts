...
type StaleRoute = '/' | `/blog/posts/${string}`

async function queryStaleRoutes(body: SanityDocument): Promise<StaleRoute[]> {
...
async function queryAllRoutes(client: SanityClient): Promise<StaleRoute[]> {
	...
	return ['/', ...slugs.map((slug) => `/blog/posts/${slug}` as StaleRoute)]
}
...
async function queryStaleAuthorRoutes(
	...
	if (slugs.length > 0) {
		...
		return ['/', ...slugs.map((slug) => `/blog/posts/${slug}`)]
	}
...
async function queryStalePostRoutes(
	...
	let slugs = await client.fetch(groq`*[_type == "post" && _id == $id].slug.current`, {
		id
	})
