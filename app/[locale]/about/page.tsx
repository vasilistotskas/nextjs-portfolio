...
function AboutContent() {
	...
	return (
		<div className="mx-auto max-w-5xl px-4 py-8 md:px-6 md:py-20">
			{/* Header with avatar */}
			<div className="mb-8 flex flex-col items-start gap-4 md:mb-14 md:flex-row md:items-center md:gap-8">
				{/* Avatar */}
				...
				{/* Title & role */}
				<div>
					<h1 className="text-terminal-text mb-2 font-sans text-3xl font-semibold tracking-tight md:text-4xl">
						{tc('myName')}
					</h1>
					<p className="text-terminal-comment font-sans text-lg">
						{t('role')} {t('companyAt')}{' '}
						<span className="text-terminal-green font-medium">{t('company')}</span>
					</p>
					{/* Its own line: inline, this wrapped and left a dangling separator. */}
					<p className="text-terminal-muted mt-1 flex items-center gap-1.5 font-sans text-sm">
				...
				<div className="space-y-8 lg:col-span-2">
					...
					<Terminal title="README.md">
						...
						<div className="space-y-5 leading-relaxed">
							...
							{/* Current Work */}
							<div>
								<h3 className="text-terminal-green mb-1.5 font-semibold">
									{t('sections.work.title')}
								</h3>
								<p className="text-terminal-text">{t('sections.work.body')}</p>
							</div>
							...
							<div>
								...
								<p className="text-terminal-text">
									...
									<a
										...
										className="text-terminal-green decoration-terminal-green/30 hover:decoration-terminal-green underline underline-offset-2 transition-colors"
									>
							...
							{/* Core Tech Stack */}
							<div>
								<h3 className="text-terminal-green mb-2.5 font-semibold">
								...
								<div className="space-y-2.5">
									{stackCategories.map(({ label, value }) => (
										<div key={label}>
											<span className="text-terminal-muted text-xs tracking-wider uppercase">
												{label}:{' '}
											</span>
							...
							<div>
								...
								<p className="text-terminal-text">{t('sections.ai.body')}</p>
							</div>

							{/* Closing */}
							<p className="text-terminal-comment italic">{t('closing')}</p>
						</div>
					</Terminal>
						...
						{highlights.map(({ icon: Icon, text, color }) => (
							<Card key={text} hover={false} className="p-4">
								<div className="flex flex-col items-center gap-3 md:flex-row">
									...
									<span className="text-terminal-text font-sans text-sm">{text}</span>
								</div>
							</Card>
						))}
					</div>
						...
						<div className="space-y-4">
							<div className="flex items-center gap-3">
								...
								<a
									...
									className="text-terminal-text hover:text-terminal-green font-sans text-sm transition-colors"
								>
									{tc('myEmail')}
								</a>
							</div>
							<div className="flex items-center gap-3">
								<GraduationCap size={14} className="text-terminal-green" />
							...
							<div className="flex items-center gap-3">
								<Award size={14} className="text-terminal-green" />
								<span className="text-terminal-text font-sans text-sm">
